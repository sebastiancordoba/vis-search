  ...
  while (openList.length > 0) {
    ...
    if (currentNode === endNode) {
      ...
      while (currentNode.parent) {
        currentNode.color = color(88, 166, 255, 150); // Path color
        path.push(currentNode);
        currentNode = currentNode.parent;
      }
      path.push(startNode);
      startNode.color = color(88, 166, 255); // Blue
      endNode.color = color(248, 81, 73); // Red
    ...
    for (let neighbor of neighbors) {
      ...
      neighbor.color = color(56, 139, 253, 40); // Visited nodes
    }
  }
